...
INSERT INTO todo (title, details, priority, created_at, completed_at)
    VALUES ('Walk the dog', 'Get little dudes leash, walk the dog, clean any    poo he creates, give the dog a treat', 5, '2017-05-29 12:00:00', null),
           ...
           ('Wash clothes', 'Put your dirty clothes in the washing machine, throw in detergent pod, close door and turn on washing machine', 5,'2017-05-29 12:00:00', CURRENT_TIMESTAMP(0)),
           ...
           ('Change timestamps on this project', 'Read the instructions wrong, start project, read instructions again, restart project', 10,'2017-05-29 12:00:00', CURRENT_TIMESTAMP(0)),
           ('Buy Iron Yard Swag', 'Go to the Iron Yard site, purchase goodies from them', 3,CURRENT_TIMESTAMP(0), null),
           ('Take a road trip', 'Agree on a place to go, load up the car, Head to agreed upon place', 9,'2017-05-29 12:00:00', null),
...
-- get a count of all incomplete todos and grouped by priority
SELECT COUNT(id), priority FROM todo WHERE completed_at IS NULL GROUP BY priority;

-- find the NUMBER of todos by priority created in the last 30 days
SELECT COUNT(id), priority, created_at FROM todo WHERE created_at > (current_date - INTERVAL '30') GROUP BY created_at, priority;

-- Find the next todo by finding the oldest created todo with the highest priority
SELECT title, priority, created_at FROM todo WHERE completed_at IS NULL ORDER BY  created_at ASC, priority DESC;
